...
def parse_pair_id(pair_id: str) -> Tuple[str, str]:
    """
    Parse a normalized pair ID into two language codes.
    """
...
class SwitchLinguaProcessor:
    ...
    def _filter_language_pairs(self, language_pairs: List[str]):
        ...
        for pair_id in language_pairs:
            lang1, lang2 = parse_pair_id(pair_id.strip().lower())
            normalized_targets.add(normalize_pair_id(lang1, lang2))
        ...
        for split_name, split_data in self.dataset.items():
            ...
            for row in split_data:
                row_lang1 = row.get("first_language", "unknown")
                row_lang2 = row.get("second_language", "unknown")
                row_pair_id = normalize_pair_id(row_lang1, row_lang2)

                if row_pair_id in normalized_targets:
                    kept_rows.append(row)

            filtered_dataset[split_name] = kept_rows
            print(
                f"  Split '{split_name}': kept {len(kept_rows)} examples "
                f"for requested pairs {sorted(normalized_targets)}"
            )

        self.dataset = filtered_dataset
        print("✓ Language-pair filtering complete")
    ...
    def process_examples(self, split: str = 'train', max_examples: Optional[int] = None):
        ...
        for idx in range(num_examples):
            try:
                ...
                pair_id = normalize_pair_id(first_language, second_language)
...
if __name__ == "__main__":
    ...
    processor.process_examples(split='train')
