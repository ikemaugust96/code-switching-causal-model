...
def summarize_rows(rows):
    ...
    for row in rows:
        ...
        grouped[model_name]["duration_accuracy"].append(
            row.get("duration_accuracy", 0.0)
        )
        grouped[model_name]["duration_f1_macro"].append(
            row.get("duration_f1_macro", 0.0)
        )
        ...
        summary.append(
            {
                ...
                "avg_switch_accuracy": sum(stats["switch_accuracy"])
                / max(1, len(stats["switch_accuracy"])),
                "avg_switch_f1": sum(stats["switch_f1"])
                / max(1, len(stats["switch_f1"])),
                "avg_duration_accuracy": sum(stats["duration_accuracy"])
                / max(1, len(stats["duration_accuracy"])),
                "avg_duration_f1_macro": sum(stats["duration_f1_macro"])
                / max(1, len(stats["duration_f1_macro"])),
            }
        )
...
def run_training_experiment(
    ...
    experiment_name="universality",
):
    """
    Train the existing GRU pipeline on a provided train/test split.
    This adapts the current proposed_model.py code to the universality script.
    """
    set_seed(seed)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\nRunning GRU experiment: {experiment_name}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if results["switch_f1"] > best_switch_f1:
            ...
            best_results["num_position_samples"] = len(test_dataset)
            best_results["num_duration_samples"] = sum(
                1 for sample in test_dataset.samples if sample["duration_label"] != -1
            )
